...
const createPropertyDataForAgent = (propertyData) => {
  ...
  // ✅ NEW: Get real creation date from XML
  const createdDate = parseCreatedAtDate(propertyData);
  
  // ✅ NEW: Get last update date (timestamp)
  const updatedDate = propertyData.timestamp 
    ? new Date(propertyData.timestamp) 
    : new Date();

  return {
    propertyId: propertyData.id || propertyData.Id,
    listingTitle: generalInfo.listing_title || 'No Title',
    ...
    images: propertyData.listing_media?.images?.image || [],
    description: generalInfo.description || '',
    // ✅ UPDATED: Use real created_at date from XML
    addedDate: propertyData.created_at || "0",
...
const parseCreatedAtDate = (propertyData) => {
  try {
    // XML attribute: created_at="2025-07-08 12:53:19"
    if (propertyData.created_at) {
      const dateStr = propertyData.created_at;
    ...
    if (propertyData.timestamp) {
      const date = new Date(propertyData.timestamp);
      if (!isNaN(date.getTime())) {
        return date;
      }
    }
    ...
    console.warn(`No valid created_at date found for property ${propertyData.Id || 'unknown'}, using current date`);
    return new Date();
