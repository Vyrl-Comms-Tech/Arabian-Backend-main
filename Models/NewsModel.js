...
async function generateUniqueSlug(title, NewsModel, excludeId = null) {
  ...
  while (true) {
    ...
    const existingNews = await NewsModel.findOne(query);
    if (!existingNews) {
      break;
    }
...
// Static method to search news by content
