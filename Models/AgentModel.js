...
const agentSchema = new mongoose.Schema({
  agentId: {
    type: String,
    unique: true,
    default: () => `AGENT_${uuidv4().replace(/-/g, '').substring(0, 8).toUpperCase()}`,
    required: true
  },
...
agentSchema.virtual('activeProperties').get(function() {
  return this.properties ? this.properties.filter(p => p.status !== 'Off Market').length : 0;
});

agentSchema.virtual('saleProperties').get(function() {
  return this.properties ? this.properties.filter(p => p.listingType === 'Sale').length : 0;
});

agentSchema.virtual('rentProperties').get(function() {
  return this.properties ? this.properties.filter(p => p.listingType === 'Rent').length : 0;
});

agentSchema.virtual('offPlanProperties').get(function() {
  return this.properties ? this.properties.filter(p => p.listingType === 'Off Plan').length : 0;
});
...
agentSchema.statics.getNextSequenceNumber = async function() {
  const lastAgent = await this.findOne({}, {}, { sort: { sequenceNumber: -1 } });
  return lastAgent && lastAgent.sequenceNumber ? lastAgent.sequenceNumber + 1 : 1;
};

agentSchema.statics.swapSequenceNumbers = async function(agentId, newSequenceNumber) {
  ...
  try {
    await session.withTransaction(async () => {
      ...
      if (targetAgent) {
        ...
        console.log(`✅ Swapped sequence numbers: Agent ${currentAgent.agentName} (${currentSequence} → ${newSequenceNumber}), Agent ${targetAgent.agentName} (${newSequenceNumber} → ${currentSequence})`);
      } else {
        ...
        console.log(`✅ Updated sequence number: Agent ${currentAgent.agentName} → ${newSequenceNumber}`);
      }
    });
  } catch (error) {
...
agentSchema.methods.addOrUpdateBlog = function(blogData) {
  ...
  if (existingBlogIndex > -1) {
    ...
    this.blogs[existingBlogIndex].createdAt = blogData.createdAt || this.blogs[existingBlogIndex].createdAt;
    this.blogs[existingBlogIndex].updatedAt = new Date();
    
    console.log(`✅ Updated existing blog: ${blogData.title} with image: ${blogData.image?.filename}`);
  } else {
    ...
    console.log(`✅ Added new blog: ${blogData.title} with image: ${blogData.image?.filename}`);
  }
...
agentSchema.methods.removeBlog = function(blogId) {
  ...
  this.blogs = this.blogs.filter(b => b.blogId.toString() !== blogId.toString());
...
agentSchema.statics.findActiveAgents = function() {
  return this.find({ isActive: true }).sort({ sequenceNumber: 1, agentName: 1 });
};
